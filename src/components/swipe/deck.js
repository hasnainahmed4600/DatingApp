...
const Deck = props => {
  ...
  const onLike2Pressed = (isSwipeGesture = false) => {
    const { onSwipe2Right, data } = props;
    const item = data[currentIndex.current];

    Animated.spring(position.current, {
      toValue: { x: SCREEN_WIDTH + 100, y: 0 }
    }).start(() => {
      onSwipe2Right(item, currentIndex.current, isSwipeGesture);
      position.current.setValue({ x: 0, y: 0 });
    });
  };
    ...
    return (
      <View style={{ marginBottom: -8 }}>
        <BottomTabBar
          ...
          onSuperLikePressed={onLike2Pressed}
        ...
        if (i < currentIndex.current) {
          ...
          return (
            ...
            >
              ...
              >
                <Text style={styles.likeText}>{IMLocalized("+CHIRP")}</Text>
              </Animated.View>
              ...
              >
                <Text style={styles.nopeText}>{IMLocalized("Not Today")}</Text>
              </Animated.View>
              {renderCard(item)}
            </Animated.View>
          );
