...
const SwipeScreen = props => {
  ...
  firestoreUsers = useSelector(state => state.auth.user);
  ...
  const recommendationBatchLimit = useRef(25);
  ...
  const recommendationRef = useRef(
    usersRef.orderBy("id", "desc").limit(recommendationBatchLimit.current)
  );
  ...
  const handleAndroidLocationPermission = async () => {
    try {
      const granted = await PermissionsAndroid.request(
        ...
        {
          title: IMLocalized("Parrot"),
          message: IMLocalized("Parrot wants to access your location ")
        }
      );
  ...
  const handleIncompleteUserData = () => {
    Alert.alert(
      ...
      IMLocalized(
        "Welcome to Parrot. Let's complete your profile to let other people find you."
      ),
      [
        {
  ...
  const filterUsersForRecommendation = async (otherUser, myPosition) => {
    ...
    const appDistance = (user.settings &&
      user.settings.maximum_distance &&
      user.settings.maximum_distance.split(" ")) || ["10000"];
    const distanceValue = Number(appDistance[0]);
    const { firstName, email, phone, profilePhoto, userID, id } = otherUser;
    const gender = otherUser.settings ? otherUser.settings.gender : "none";
    const location = otherUser.location
      ? otherUser.location
      : otherUser.position;
    const isNotCurrentUser =
      (userID ? userID : id) != (user.id ? user.id : user.userID);
    const sameSchool = otherUser.school == user.school ? true : false;
    const hasNotBeenBlockedByCurrentUser = !bannedUserIDs.includes(
      id || userID
    );
    const hasPreviouslyNotBeenSwiped = !swipes.find(user => user.id == userID);
    const isGenderCompatible =
      genderPre == "all" || genderPre == "Both)" ? true : gender == genderPre;
    const otherUserProfileIsPublic =
      otherUser.settings && otherUser.settings.show_me != null
        ? otherUser.settings.show_me
        : true;
  ...
  const getMoreRecommendations = async () => {
    ...
    const shouldFetchRecommendations =
      recommendations.length < swipeThreshold.current;
  ...
  const updateRecommendations = async documentSnapshots => {
    ...
    try {
      const recommendationPromises = documentSnapshots._docs.map(async doc => {
        const recommendation = await filterUsersForRecommendation(
          doc.data(),
          myLocation
        );
      ...
      const cleanedRecommendations = promisedRecommendations.filter(
  ...
  const renderCardDetail = (item, isDone) => {
    return (
      <CardDetailsView
        ...
        distance={item.distance}
        bio={item.bio}
        instagramPhotos={item.featuredPhoto ? item.featuredPhoto : []}
        setShowMode={setShowMode}
        onSwipe={direction => {
  ...
  const renderEmptyState = () => {
    return <NoMoreCard profilePhoto={user.profilePhoto} />;
  };

  const renderNewMatch = () => {
    return (
      <NewMatch
        url={currentMatchData.profilePhoto}
        ...
        <View style={styles.container}>
          {(loadedDeck || recommendations.length > 0) && (
            <Deck
              ...
              onSwipe2Right={(item, index, isSwipeGesture) =>
                onSwipe("superlike", item, index, isSwipeGesture)
              }
              showMode={showMode}
            />
          )}
