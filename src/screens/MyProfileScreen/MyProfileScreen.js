...
const MyProfileScreen = props => {
  ...
  const updatePhotos = featuredPhoto => {
    let myUpdatePhotos = [];
    let pphotos = featuredPhoto ? [...featuredPhoto] : [];
    let temp = [];
  ...
  useEffect(() => {
    if (props.user) {
      updatePhotos(props.user.featuredPhoto);
    }
  ...
  const detail = () => {
    props.navigation.navigate("AccountDetails", {
      ...
      screenTitle: IMLocalized("Edit Profle")
    });
  };
  ...
  const startUpload = source => {
    ...
    uploadPromise(source)
      .then(url => {
        const { featuredPhoto } = props.user;
        let pphotos = featuredPhoto ? featuredPhoto : [];
        pphotos.push(url);
        const data = {
          featuredPhoto: pphotos
        };
  ...
  const onPhotoDialogDone = actionSheetActionIndex => {
    ...
    if (selectedItemIndex == -1 || selectedItemIndex >= featuredPhoto.length) {
      return;
    }

    if (actionSheetActionIndex == 0) {
      if (featuredPhoto) {
        featuredPhoto.splice(selectedItemIndex, 1);
      }
      updateUserInfo({ featuredPhoto });
      updatePhotos(featuredPhoto);
    }

    if (actionSheetActionIndex == 2) {
      const photoToUpdate = featuredPhoto[selectedItemIndex];
      updateUserInfo({ profilePhoto: photoToUpdate });
    }
  };
  ...
  const { firstName, lastName, profilePhoto } = props.user;
          ...
          <ScrollView style={styles.body}>
            ...
            <TouchableOpacity style={styles.optionView} onPress={detail}>
              ...
              <View style={styles.textView}>
                <Text style={styles.textLabel}>{IMLocalized("Edit Profile")}</Text>
              </View>
            </TouchableOpacity>
            ...
            <TouchableOpacity style={styles.optionView} onPress={contact}>
              ...
              <View style={styles.textView}>
                <Text style={styles.textLabel}>{IMLocalized("Support")}</Text>
              </View>
            </TouchableOpacity>
            <TouchableOpacity style={styles.logoutView} onPress={onLogout}>
              <Text style={styles.whiteColor}>{IMLocalized("Logout")}</Text>
            </TouchableOpacity>
          </ScrollView>
          <ActionSheet
