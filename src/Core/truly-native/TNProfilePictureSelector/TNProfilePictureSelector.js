...
const TNProfilePictureSelector = props => {
  const [profilePhoto, setProfilePictureURL] = useState(props.profilePhoto || '');
    ...
    <>
      <View style={styles.imageBlock}>
        <TouchableHighlight
          ...
          onPress={() => handleProfilePictureClick(profilePhoto)}
        >
          <FastImage
            style={[styles.image, { opacity: profilePhoto ? 1 : 0.3 }]}
            source={profilePhoto ? { uri: profilePhoto } : appStyles.iconSet.userAvatar}
            resizeMode="cover"
