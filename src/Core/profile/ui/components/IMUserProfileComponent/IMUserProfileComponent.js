...
const IMUserProfileComponent = props => {
  ...
  const { profilePhoto, firstName, lastName, fullname, userID } = props.user;
      ...
      firebaseAuth
        ...
        .then(finalRes => {
          if (finalRes.success == true) {
            onUpdateUser({ ...props.user, profilePhoto: null });
          }
        })
      return;
    }
      ...
      .then(response => {
        if (response.error) {
          ...
          firebaseAuth
            ...
            .then(finalRes => {
              if (finalRes.success == true) {
                onUpdateUser({ ...props.user, profilePhoto: response.downloadURL });
              }
            })
        }
      })
