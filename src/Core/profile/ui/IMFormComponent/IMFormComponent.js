...
function IMFormComponent(props) {
  ...
  const onCustomFormFieldValueChange = (formField, value) => {
    var newFieldsDict = alteredFormDict;
    value = value.replace(/\D/g, '');
    switch (value.length) {
      case 0:
        newFieldsDict[formField.key] = "";
        setAlteredFormDict(newFieldsDict);
        break;
      case 1:
      ...
      case 2:
        ...
        break;
      case 3:
        newFieldsDict[formField.key] = value.substring(0, 2) + "/" + value.substring(2);
        setAlteredFormDict(newFieldsDict);
        break;
      case 4:
        newFieldsDict[formField.key] = value.substring(0, 2) + "/" + value.substring(2, 4);
        setAlteredFormDict(newFieldsDict);
        break;
      case 5:
        newFieldsDict[formField.key] = value.substring(0, 2) + "/" + value.substring(2, 4) + "/" + value.substring(4);
        setAlteredFormDict(newFieldsDict);
        break;
      case 6:
        newFieldsDict[formField.key] = value.substring(0, 2) + "/" + value.substring(2, 4) + "/" + value.substring(4, 8);
        setAlteredFormDict(newFieldsDict);
        break;
      case 7:
        newFieldsDict[formField.key] = value.substring(0, 2) + "/" + value.substring(2, 4) + "/" + value.substring(4, 8);
  ...
  const onEmailFormFieldValueChange = (formField, value) => {
    var newFieldsDict = alteredFormDict;
    newFieldsDict[formField.key] = value;
    setAlteredFormDict(newFieldsDict);
    onFormChange(newFieldsDict);
  ...
  const renderCustomTextField = (formTextField, index, totalLen) => {
    return (
      <View>
        <View
          style={[
          ...
          <TextInput
            ...
            style={[styles.text, { textAlign: 'right' }]}
            editable={formTextField.editable}
            onChangeText={(text) => { onCustomFormFieldValueChange(formTextField, text) }}
            placeholderTextColor={styles.placeholderTextColor}
            placeholder={formTextField.placeholder}
            value={computeValue(formTextField)}
          />
  ...
  const renderEmailField = (formTextField, index, totalLen) => {
    return (
      <View>
        <View
          style={[
          ...
          <TextInput
            ...
            style={[styles.text, { textAlign: 'right' }]}
            editable={formTextField.editable}
            onChangeText={(text) => { onEmailFormFieldValueChange(formTextField, text) }}
            placeholderTextColor={styles.placeholderTextColor}
            placeholder={formTextField.placeholder}
            value={computeValue(formTextField)}
          />
  ...
  const renderField = (formField, index, totalLen) => {
    ...
    if (type == 'customText') {
      return renderCustomTextField(formField, index, totalLen);
    }
    if (type == 'email') {
      return renderEmailField(formField, index, totalLen);
    }
    if (type == 'switch') {
