...
export const register = (userDetails, appIdentifier) => {
  const { email, firstName, lastName, password, phone, birthday, school, profilePhoto, age, zodiac, location, signUpLocation } = userDetails;
...
export const registerWithPhoneNumber = (userDetails, smsCode, verificationID, appIdentifier) => {
  const { firstName, lastName, phone, profilePhoto, groups, location, signUpLocation } = userDetails;
  const credential = firebase.auth.PhoneAuthProvider.credential(verificationID, smsCode);
  return new Promise(function (resolve, _reject) {
    firebase
      .auth()
